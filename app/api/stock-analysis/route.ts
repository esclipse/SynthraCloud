...
const normalizeStrategyInput = (strategy?: string) => {
  ...
  if (strategy === 's4' || strategy.includes('B1')) return 's4';
  if (strategy === 's5' || strategy.includes('启动K')) return 's5';
  if (strategy === 'strategy1') return 's1';
  if (strategy === 'strategy2') return 's2';
  if (strategy === 'strategy3') return 's3';
  if (strategy === 'strategy4') return 's4';
  if (strategy === 'strategy5') return 's5';
  return 'all';
};
...
const toResultKey = (strategy: string) => {
  ...
  if (strategy === 's4') return 'strategy4';
  if (strategy === 's5') return 'strategy5';
  return 'all';
};
...
const buildMatch = (stock: Record<string, any>, strategyLabel?: string) => {
  ...
  return {
    ...
    swing_pct: toNumber(pickValue(stock, ['swing_pct', '前3日振幅(%)'])),
...
const convertCloudFunctionResult = (
  cloudResult: any,
  strategyKey: 'strategy1' | 'strategy2' | 'strategy3' | 'strategy4' | 'strategy5' | 'all'
) => {
  ...
  if (strategyKey === 'all') {
    ...
    const strategy4Results = (results.strategy4 || []).map((stock: Record<string, any>) =>
      buildMatch(stock, '策略4: B1选股策略')
    );

    const strategy5Results = (results.strategy5 || []).map((stock: Record<string, any>) =>
      buildMatch(stock, '策略5: 启动K选股')
    );
